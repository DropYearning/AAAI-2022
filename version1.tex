...
\usepackage[linkbordercolor={1 0 0}, citebordercolor={0 1 0}]{hyperref}
% \usepackage[colorlinks,
%             linkcolor=blue,
%             anchorcolor=green,
%             citecolor=blue,
%             citebordercolor={0 1 0},
%             linkbordercolor={1 0 0},]{hyperref}
% \hypersetup{
%     colorlinks=true,
%     linkcolor=blue,
%     filecolor=magenta,      
...
\title{Associative Embedding for Visual Question Answering and Image Captioning}
\author{
    %Authors
    % All authors must be in the same font size and format.
    Pengbing Gao\textsuperscript{\rm 1, \rm 2}\thanks{Corresponding Author},
    Zhou Yu\textsuperscript{\rm 1},
    Jun Yu\textsuperscript{\rm 1},
    Hongwei Yang\textsuperscript{\rm 2},
    Zhenhua Guo\textsuperscript{\rm 2},
    Fangbo Tao\textsuperscript{\rm 2}
    % \equalcontrib
}
\affiliations{
    %Afiliations
    ...
    \textsuperscript{\rm 1}School of Computer Science and Technology, Hangzhou Dianzi University, China \\
    \textsuperscript{\rm 2}Alibaba Group, Hangzhou, China

    % email address must be in roman text type, not monospace or sans serif
    \{nbgao, zhouyu, yujun\}@hdu.edu.cn, \{luofu.yhw, mianzhang.gzh, fangbo.tfb\}@alibaba-inc.com
...
\begin{abstract}
In the past few years, despite region-based features extracted by object detector have been widely employed in various multi-modal tasks. However, there are still some obvious defects of region feature, such as lack of global context information and fine-grained details from paired natural languages. Moreover, annotating the high quality object labels is expensive and training the object detector is time-consuming. A series of visual-language pre-training (VLP) works, directly utilize typical region or grid features which is focus on single-modal task, lack of latent multi-modal semantic information. To tackle the above issues, we further take full advantage of the enhanced and aligned semantic information of vision and text. Without explicit pre-train task like other VLP paradigms, our method only leverages strong associative embedding of CLIP model and combines with task-specific head (model). We show how to effectively apply the powerful representation capability of associative embedding to visual question answering and image captioning. Extensive comparative experimental results and ablation studies certify that associative embedding is essential and powerful in multi-modal tasks, and this advanced feature enables to replace typical region-based or grid-based feature. Our method significantly outperforms existing VLP approaches and achieves new state-of-the-art results across two popular multi-modal tasks, including visual question answering and image captioning. In particularly, our best VQA single model delivers 76.65\% overall accuracy on the VQA-v2 test-dev set and best image captioning model achieves 139.08 CIDEr metric on MS-COCO caption Karpathy test set.
\end{abstract}


% S1: Introduction
\section{Introduction}
Most existing work on vision-language representation learning fall into two categories. The first category focuses on modelling the interactions between image and text features with transformer based multimodal encoders [10, 11, 12, 13, 1, 14, 15, 2, 3, 16, 8, 17, 18]. Methods in this category achieve superior performance on downstream V+L tasks that require complex reasoning over image and text (e.g. NLVR2 [19], VQA [20]), but most of them require high-resolution input images and pre-trained object detectors. A recent method [21] improves inference speed by removing the object detector, but results in lower performance. The second category focuses on learning separate unimodal encoders for image and text [22, 23, 6, 7]. The recent CLIP [6] and ALIGN [7] perform pre-training on massive noisy web data using a contrastive loss, one of the most effective loss for representation learning [24, 25]. They achieve remarkable performance on image-text retrieval tasks, but lack the ability to model more complex interactions between image and text for other V+L tasks [21]. ALBEF unifies the two categories, leading to strong unimodal and multimodal representations with superior performance on both retrieval and reasoning tasks. Furthermore, ALBEF does not require
object detectors, a major computation bottleneck for many existing methods

The disadvantages of region feature
Despite the great success, region features are still criticized for the lack of global contextual information and fine-grained details.

Associative embedding is pivotal in cross-modal learning, because strong associative embedding learned the alignment semantic. project vision feature and language feature to the unified semantic space - associative space.

Associative embedding facilitates cross-modal semantic alignment and comprehension. 
benefit down-stream multi-modal tasks abundant cross-modality information

CLIP-RN50x16 with MMNAS model achieves the best performance of 76.65\% on test-dev split and 76.75\% on test-std split.

We summarize the contributions of this paper as follows:
\begin{itemize}
    \item We propose
    \item Associative embedding perspective both leverage vision encoder and text encoder of CLIP model
    \item latent VLP
    \item Do not need object detector to extract region features.
\end{itemize}

Figures ~\ref{fig-1}, and  display
...
% 3) Multimodal Tasks (Learning)
\subsubsection{Visual Representation \& Language Representation.}
Unimodel representation
Visual representation learning for vision-language understanding
is a long-standing research topic. Many early works
use CNN image classification models pre-trained on ImageNet to extract visual features such as VGGNet, ResNet, ResNeXt, etc. 

Later on, Bottom-Up and Top-Down Attention (BUTD) \cite{butd} to extract region-level feature by Faster R-CNN \cite{frcn} object detection model pre-trained on Visual Genome dataset \cite{vg}, these region-based features as the visual inputs for visual question answering and image captioning tasks, significantly improving the quantitative performance for multimodal tasks.  Recently, VinVL \cite{vinvl} use better-designed region-level features for VL tasks, and is bigger and trained on much larger amounts of data, combining four public object detection datasets, including MS-COCO \cite{mscoco}, OpenImages (OI) \cite{openimages}, Objects365 \cite{objects365} and Visual Genome \cite{vg}.

Recently, some works propose to directly learn visual representations in the form of grid features with convolutional networks in specific vision language tasks [17] or vision recognition tasks [9, 33]. grid features interest show the impact of 


Our work shares a similar format of visual representation
with [17] while we focus on the area of vision-language
pre-training and propose the first end-to-end VLPT model
without relying on the box annotations.
\cite{butd} first use region feature with Faster R-CNN \\
\cite{grid-vqa} use grid feature
To extract visual features, we use the pre-trained Faster-
RCNN (Ren et al. 2015) provided by \cite{grid-vqa} that
uses delated stride-1 C5 backbone and $ 1\times1 $ RoIPool with
two FC layers as the detection head to train Faster R-CNN
on the VG dataset. In the feature extraction stage, it removes
the delation and uses a normal C5 layer to extract grid features.
For grid features, we leverage their grid features and

\subsubsection{Vision-Language Pre-training (VLP).} Uni-modal Pretraining \\
Visual-and-Language Pretraining (VLP) \\
One-stage vision-language pre-training methods
Pixel BERT SOHO E2E-VLP  usually leverage grid feature. These methods are end-to-end trainable, which means they can learn from pixel.\\
Two-stage vision-language pre-training methods \\
Recent years have seen a rapid progress made in vision-language pretraining (Uppal et al., 2020;
Han et al., 2021; Khan et al., 2021). While a variety of approaches have been proposed, a large portion of them require object detection for image region feature regression or tagging as part of the pre-training objectives,
single-stream architecture: UNITER \cite{uniter} OSCAR \cite{oscar} ROSITA \cite{rosita} \\
Two-stream architecture: ViLBERT, LXMERT, ERNIE-ViL \\
 
Recently, several contrastive learning-based vision-language pre-training methods have been proposed. OpenAI CLIP \cite{clip} is the typical example, it leverages large-scale image-text pairs to learn transferrable visual representations by image-text matching, which achieves excellent performance on zero-shot image classification and image-text retrieval tasks.

\subsubsection{multi-modal Tasks (Learning).}
multi-modal learning aims to build
models that can understand and associate information from multiple
modalities. From early research on audio-visual speech
recognition [12, 60] to the recent explosion of interest in vision-andlanguage tasks [2, 8, 54], multi-modal learning is a multi-disciplinary field of significant importance and potential. At present, multi-modal learning with deep neural networks is the de facto paradigm for modern multi-modal learning tasks, such as visual question answering (VQA), image caption and image-text matching. In the following, we briefly describe three typical multi-modal learning tasks and a few representative approaches accordingly.
The VQA task aims to answer a question in natural language
with respect to a given image, which requires a fine-grained and
simultaneous understanding of both image and question. Antol et al.
present a large-scale VQA benchmark with human annotations and
some baseline methods [2]. Fukui et al. [14], Kim et al. [27], Ben et al.[4], and Yu et al. [57] devise different approximated bilinear pooling models to effectively fuse multi-modal features with second-order interactions and then integrate them with attention-based neural networks. Most recently, deep co-attention models are proposed to integrate multi-modal fusion and attention learning and deliver new state-of-the-art performance on the benchmark datasets [15, 26, 37]. Yu et al. introduce the idea of modularization into the deep co-attention model to characterize the fine-grained multi-modal interactions by modularized attention units [56].

Most of the recent works on image captioning are built upon (Anderson et al. 2018), where a language model gets clues for sentence generation through dynamically attending on object regions in the image extracted from pre-trained object detectors. Follow-up
works further capture the relationships among object regions
by using Graph Convolutional Networks (GCNs) (Yao et al.
2018), incorporating language inductive bias (Yang et al.
2019), or enforcing region grounding between image and
text (Lu et al. 2018; Zhou et al. 2019). \\
VQA is another prevalent research area in vision and language. Since its initial proposal (Antol et al. 2015), there has been a significant amount of works proposing model architectures to fuse
question and image representations (Kim, Jun, and Zhang
2018; Anderson et al. 2018; Gao et al. 2019), new datasets
or models to reduce the dataset bias (Zhang et al. 2016;
Goyal et al. 2017; Agrawal et al. 2017) and ground the answer
in the question (Lewis and Fan 2019).
...
\begin{tabular}{c|c|c c c c}
    \toprule
    Task & Dataset & \tabincell{c}{Image\\Source} & \#Img. & \#Sent. & Metric \\ \midrule
    VQA & VQA-v2 & COCO & 204K & 1.1M & Accuracy \\ %\midrule
    Caption & COCO & COCO & 123K & 616K & B, M, R, C, S \\ 
    % ITM & Flickr30K & Flickr & 31K & 155K & Recall@K \\ %\midrule
    \bottomrule
\end{tabular}
\caption{The detailed statistics and evaluation metrics of the
tasks and datasets.}
\label{table-1}
\end{table}
...
For image caption, following the standard evaluation criterion, we utilize BLEU@N \cite{bleu}, METEOR \cite{meteor}, ROUGE-L \cite{rouge}, CIDEr \cite{cider}, and SPICE \cite{spice} to evaluate our model.
...
\toprule
    \multirow{2}{*}{Method} & \multirow{2}{*}{Feature} & \multicolumn{4}{c|}{Test-Dev} & Test-Std \\ \cmidrule{3-7}
     & & All & Y/N & Num & Other & All \\ \midrule
    MCAN    & region & 70.63 & 86.82 & 53.26 & 60.72 & 70.90 \\
    MMNAS   & region & 71.24 & 87.27 & 55.68 & 61.05 & 71.46 \\ \midrule
    LXMERT  & region & 72.42 &  &  &  & 72.54 \\
    UNITER  & region & 72.70 &  &  &  & 72.91 \\
    ERNIE-ViL& region &  72.62 &  &  &  & 72.85 \\
    OSCAR   & region & 73.16 &  &  &  & 73.44 \\
    VILLA   & region & 73.59 &  &  &  & 73.67 \\
    UNIMO   & region & 73.79 &  &  &  & 74.02 \\
    ROSITA  & region &  73.91 &  &  &  & 73.97 \\
    VinVL   & region &  75.95 &  &  &  & 76.12 \\ \midrule
    SOHO    & grid & 73.25 &  &  &  & 73.47 \\
    E2E-VLP & grid & 73.25 &  &  &  & 73.67 \\ 
    Pixel-BERT & grid & 74.45  &  &  &  & 74.55 \\ \midrule
    AEMM(ours) & grid & 76.65 & 91.27 & 61.64 & 67.55 & 76.75 \\ 
    \bottomrule
\end{tabular}
%}
\caption{Accuracies on the test-dev and test-std splits of VQAv2.}
\label{table-2}
\end{table*}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/vqa_gridfeat_accuracy.pdf} % Reduce the figure size so that it is slightly narrower than the column. Don't use precise values for figure width.This setup will avoid overfull boxes.
...
Our model achieves new state-of-the-art performance for VQA and image captioning.

The experimental results show that our method greatly improves the performance of visual question answering and image captioning.

In future work, we believe that further searching for a more proper mmnas model architecture for associative embedding can obtain better performance on VQA task. On the other hand, combine these associative embedding with region features such as VinVL object features can make the model more powerful in visual-and-language tasks.



\section{Acknowledgement}
This work was supported by Alibaba Group through Alibaba Research Intern Program, and in part by the National Key R\&D Program of China under Grant 2018AAA0100603, and in part by National Natural Science Foundation of China under Grant 62072147, Grant 61836002, and Grant 62020106007.
